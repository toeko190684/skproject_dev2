...
if($_GET[data]=='reconumber'){
	...
	$kode_promo = mysql_query("select concat('RC/MKI/','$_POST[divisi]','/','$_POST[departemen]','/',substring($tgl[2],3,2),
	                           reverse(substring(reverse(concat('00',$tgl[0])),1,2)),
	                           reverse(substring(reverse(concat('0000',MAX(reverse(substring(reverse(kode_promo),1,4)))+1)),1,4)) )as kode_promo FROM RECO_REQUEST
							   where trim(reverse(substring(reverse(kode_promo),10,20))) 
							   = concat('RC/MKI/','$_POST[divisi]','/','$_POST[departemen]')");
	$rkode_promo = mysql_fetch_array($kode_promo);
	print json_encode($rkode_promo);
}
...
if($_GET[data]=='simpanreco'){
	...
	if($cek>0){
		//update data
		$sql = mysql_query("update reco_request set kode_promo='$_POST[kode_promo]',
			...
			if($sql){//jika berhasil disimpan maka 
				...
				mysql_query("delete from detail_reco_target where kode_reco='$_POST[kode_promo]'");
				...
				mysql_query("delete from detail_reco_item where kode_reco='$_POST[kode_promo]'");
                ...
                echo "Update Reco : $_POST[kode_promo] berhasil..!$_SESSION[pro_id]";
			}			
	}else{// kalau datanya masih kosong maka di simpan
	    //cek apakah kode budget sudah dimasukan di tab reco item
        $dbudget = mysql_query("select * from temp_detail_reco_item where kode_reco='$_POST[kode_promo]'");
		  ...
		  if($rcari[status]<>'pending'){
				  ...
				  if($sql){					
						if($_POST[status]=='approved'){		  
							  ...
							  $term_of_pay = "select term_of_pay from vendor where vendor_id='$rcari[vendor_id]'";
							  $qterm_of_pay = odbc_exec($conn2,$term_of_pay);
							  $rterm_of_pay = odbc_fetch_array($qterm_of_pay);
							  ...
							  $update = "INSERT INTO [kinosentraacc].[dbo].[ap_journal]([user_id]
																			 ...
																			 VALUES
																				   ...
																				   ,'$tgl_journal'
																				   ,0
																				   ,dateadd(day,$rterm_of_pay[term_of_pay],'$tgl_journal')
																				   ,0
																				   ,0
																				   ,''
																				   ,''
							  ...
							  if($ap_journal){ 
									...
									//update juga due date data yang sudah masuk ke purwadika
									$query = "update ap_journal set due_date = dateadd(dd,30,due_date) where journal_id='$journal_id'";

...
$(document).ready(function(){
	...
	$('#create').click(function(){
		...
		$.post('../function/get_data.php?data=reconumber',{id : module_id,divisi : divisi_id,departemen : department_id},function(data){
		    var obj = $.parseJSON(data);
			var number;
			...
			$('#kode_promo').val(prefix);
			
			//posting untuk hapus temporary
		    $.post('../function/get_data.php?data=del_temp',{id : prefix },function(data){
		    });
		});
	});
...
switch($_GET[act]){
  ...
  if($access=='allow'){
	...
	echo "								<button type='reset' id='reset' class='btn btn-danger' >Reset</button>
						    </div>
						</div>
     			  </div>
				  ...
				  <div class='tab-pane' id='reco_item'>
				      <div class='span6'>
						  ...
						  <div class='control-group'>
								<label class='control-label' for='budget_cost'>Budget Cost</label>
								<div class='controls'>
						  ...
						  <div class='control-group'>
								<label class='control-label' for='outstanding_cost'>Outstanding Cost</label>
								<div class='controls'>

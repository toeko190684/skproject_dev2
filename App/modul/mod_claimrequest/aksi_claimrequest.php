...
elseif ($module=='claimrequest' AND $act=='input'){
	...
	if($access=="allow"){  
	  //cari nomor 
	  $blnthn = substr(date('Y',strtotime($_POST[claimdate])),2,2).date('m',strtotime($_POST[claimdate]));
	  $sql1 = $db->query("SELECT concat('CL/MKI/','$_POST[divisi]','/','$_POST[departemen]','/','$blnthn',reverse(substring(reverse(concat('0000',
						ifnull(max(reverse(substring(reverse(claim_number_system),1,4))),0)+1)),1,4))) as urut 
						FROM `claim_request` WHERE reverse(substring(reverse(claim_number_system),5,4))=$blnthn");
	  ...
	  if($sql){
			$_SESSION[pesan] = "<strong>Inserted !!</strong>, claim request $data->urut successfully saved ! ";
...
elseif ($module=='claimrequest' AND $act=='update'){
	...
	if($access=="allow"){
		mysql_query("UPDATE claim_request set claim_number_dist='$_POST[claimnumberdist]',
			...
			$_SESSION[pesan] = "<strong>Updated Data!!</strong> , Claim Number System : $_POST[id] Successfully Update!" ;
	}
	header('location:../../index.php?r='.$module);
}
?>

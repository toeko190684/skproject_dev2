...
$(document).ready(function(){
    ...
    $('#simpan').click(function(){		
		...
		//cek apakah claim number system sudah ada atau belum
		
		$.post('modul/mod_claimrequest/get_data.php?data=ceknumber',{claimnumbersystem : claimnumbersystem, status : status },function(data){
			$('#claimnumbersystem').val(data);		
		    
			claimnumbersystem = data;
				...
				if(cekppn=='true'){
					if(nomorfakturpajak==''){
						...
						$.post('modul/mod_claimrequest/get_data.php?data=simpan_claim',{
								...
								$.messager.confirm('SKProject',data,function(r){
									if(r){
										$('#batal').click();
									}
								});			
...
/*======================================BEGIN EDIT CLAIM REQUEST =========================================================================*/	
	 ...
	 case "edit_claimrequest" :
			...
			if($access =="allow"){				
				...
				$query1 = $db->query("SELECT costofpromo+claim_approved_ammount-sum(claim_approved_ammount)as outstanding FROM claim_request 
				...
				if(strtoupper($data->status) != "PENDING"){
					...
					echo "<form class='form-horizontal' method='post' action='modul/mod_claimrequest/aksi_claimrequest.php?r=claimrequest&act=update' >
								  ...
								  <div class='span7'>
										  ...
										  <div class='control-group'>
												<label class='control-label' for='outstanding' >Maksimal Claim Value</label>
												<div class='controls'>

...
$header = "<table style='border: solid 1px #cccccc;border-padding:5px;font-size:12;font-type:arial'>
			 ...
			 <td align='center' $lebar>DO vs BSC.TARGET %</td>
			 <td align='center' $lebar>DO vs UPD.TARGET %</td>
		...
		while($rx = odbc_fetch_array($qx)){
			...
			@$dob = do_divisi($tgl_awal,$tgl_akhir,$rx[category])/basic_target($tgl_akhir,$rx[category])*100;
			...
			@$lastyear = $lastyear + target($tgl_akhir_lastyear,$rx[category])/$pembagi;
			...
			$isi = $isi."<tR>
					...
					<td align='right'>".number_format(target($tgl_akhir_lastyear,$rx[category])/$pembagi,2,',','.')."</td>
     			    <td align='right'>".number_format(so_divisi($tgl_awal,$tgl_akhir,$rx[category])/$pembagi,2,',','.')."</td>
					<td align='right' bgcolor='#FFD700'>".number_format(do_divisi($tgl_awal,$tgl_akhir,$rx[category])/$pembagi,2,',','.')."</td>
...
$user = mysql_query("select a.*,b.full_name,b.email,c.report_name,c.keterangan from email_notification a,sec_users b,master_report c 
                     where a.user_id=b.user_id and a.report_id=1 and a.report_id=c.report_id");

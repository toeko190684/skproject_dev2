...
$header = "<table style='border: solid 1px #cccccc;border-padding:5px;font-size:12;font-type:arial'>
		...
		while($rx = odbc_fetch_array($qx)){
			...
			@$so = $so + so_product($tgl_awal,$tgl_akhir,$rx[product_id])/$pembagi;
			@$do = $do + do_product($tgl_awal,$tgl_akhir,$rx[product_id])/$pembagi;
			
			$isi = $isi."<tR>
	    			<td>$rx[product_id]</td>
					<td>$rproduct[product_name]</td>
     			    <td align='right'>".number_format(so_product($tgl_awal,$tgl_akhir,$rx[product_id])/$pembagi,2,',','.')."</td>
					<td align='right'>".number_format(do_product($tgl_awal,$tgl_akhir,$rx[product_id])/$pembagi,2,',','.')."</td>
...
$user = mysql_query("select a.*,b.full_name,b.email,c.report_name,c.keterangan from email_notification a,sec_users b,master_report c 
                    where a.user_id=b.user_id and a.report_id=14 and a.report_id=c.report_id");
